...
(function($) {
	var thum = $('.thum');
	var big = $('.big');
	var thumWidth = thum.outerWidth();
	var thumHeight = thum.outerHeight();
	big.hide();
	thum.on('mousemove',function(event) {
		// console.log(event.offsetX + ', ' + event.offsetY);

		//  vw 환산하는 방법: 300이미지를 1280기준 -> vw환산 
		//  현재크기 / 기준브라우저 * 100
    ...
    big.fadeIn();
    thum.css({cursor:'crosshair'});
		big.css({backgroundPosition:percentX + '% ' + '  ' + percentY + '%'});
	});

	thum.on('mouseleave',function() {
		big.fadeOut();
	});
})(this.jQuery);
